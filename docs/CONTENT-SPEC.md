...
- Visual types: `image`, `pictograph`, `compass-map`, `dice`, `polygon`, `row` (pictures left to right), `clock` (hour 1-12, minute 0/30), `table` (name + count), `number-row` (numbers with one `?`), `shape-count` (3-9 triangle/circle/square), all drawn in `src/visuals/visuals.js`.
- Picture-free options can use `svg: { fold: 'heart' | 'crescent' | 'lshape' | 'star' | 'circle' | 'flag' }`, drawn with a dashed fold line for symmetry questions.
